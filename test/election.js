...
contract("Election", function(accounts){
    ...
    it('throws an exception for invalid candidates', function() {
        return Election.deployed().then( function(i){
            ...
            var voteCount = candidate[2];
            assert.equal(voteCount, 0, "candiate 2 did not recive any vote");
        });
    });
    it('throws an exception for double voting', function() {
        return Election.deployed().then(function(i){
            ...
            var voteCount = candidate[2];
            assert.equal(voteCount, 1, "accept the first vote");
            ...
            assert.equal(voteCount, 1, "candidate 1 did not recive any votes");
            return electionInstance.candidates(2);
        }).then(function(candidate2){
            var voteCount=candidate2[2];
            assert.equal(voteCount, 1, "candiate 2 did not recive any votes");
        });
    });
});